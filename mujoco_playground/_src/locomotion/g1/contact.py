  ...
  return config_dict.create(
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              stance_reward=1.7,
              swing_reward=0.2,
              touchdown_reward=-0.5,
              touchdown_finegrained_reward=3.0,
              new_goal_reward=1.8,
...
class ContactEnv(g1_base.G1Env):
  ...
  def _generate_contact_plan(self):
    ...
    pattern = jp.array([[True, True], [True, False], [True, True], [False, True]])
  ...
  def _generate_future_ee_positions(self, rng: jax.Array, data: mjx.Data, contact_plan: jax.Array) -> jax.Array:
    ...
    # left_foot_pos = data.site_xpos[self._feet_site_id][0].at[2].set(0.0)
    # right_foot_pos = data.site_xpos[self._feet_site_id][1].at[2].set(0.0)
    ...
    #    The Z-axis is typically the 3rd component (index 2).
    projected_base_pos = base_pos.at[2].set(0.0)

    # 3. Get the robot's right and left vectors relative to its current orientation
    #    data.xmat is a 3x3 rotation matrix for each body.
    #    The columns of data.xmat represent the body's local X, Y, Z axes in world coordinates.
    #    Conventionally:
    #      Column 0: Local X-axis (forward)
    #      Column 1: Local Y-axis (left)
    #      Column 2: Local Z-axis (up)
    
    # So, the world-frame vector pointing to the robot's local left is data.xmat[:, 1]
    robot_left_vector_world = data.xmat[self._torso_body_id, :3, 1]
    
    # And the world-frame vector pointing to the robot's local right is the negative of that
    robot_right_vector_world = -data.xmat[self._torso_body_id, :3, 1]

    # Define the desired offset magnitude
    offset_magnitude = 0.12 # As requested, 0.12 to the left/right
    ...
    # Stack them to form the initial foot_positions array
    foot_positions = [initial_left_foot_pos, initial_right_foot_pos] # This will be unpacked later
    ...
    robot_forward_vector_world = data.xmat[self._torso_body_id, :3, 0]

    direction = robot_forward_vector_world.at[2].set(0.0) # Set Z-component to 0
    direction = direction / jp.linalg.norm(direction) # Normalize to a unit vector
...
#     )
  ...
  #   condition = jp.logical_and(is_in_rectangle, jp.all(info["contact_status"] == info["contact_plan"][info["current_goal_index"]]))

  #   return condition
  
  def _is_goal_reached(self, info: Dict) -> jax.Array:
    robot_base_pos_w = info["robot_base_pos_w"]
    robot_base_pos_proj = robot_base_pos_w.at[2].set(0.0)

    goal_point_1 = info["desired_ee_positions_w"][0].at[2].set(0.0)
    goal_point_2 = info["desired_ee_positions_w"][1].at[2].set(0.0)

    dist_to_goal1 = jp.linalg.norm(robot_base_pos_proj - goal_point_1)
    dist_to_goal2 = jp.linalg.norm(robot_base_pos_proj - goal_point_2)

    
    proximity_condition = (dist_to_goal1 <= 0.18) | \
                              (dist_to_goal2 <= 0.18)
    
    touchdown_phase = info["time_left"] <= self._config.command_config.touchdown_threshold
    condition = jp.logical_and(proximity_condition, touchdown_phase)
    
    # is_correct_contact = jp.all(info["contact_status"] == info["contact_plan"][info["current_goal_index"]])
  ...
  def _compute_command(self, info: Dict) -> jax.Array:
    # jax.debug.print("computing command for step: {}", info["step"])
    info["time_left"] = info["time_left"] - self._config.ctrl_dt
    info["previous_goal_index"] = jp.roll(info["previous_goal_index"], shift=1)
    info["previous_goal_index"] = info["previous_goal_index"].at[0].set(jp.where(info["current_goal_index"] == -1, 0, info["current_goal_index"]))
  ...
  def _resample_command(self, info: Dict) -> Dict:
    # jax.debug.print("resampling command for step: {}", info["step"])
    new_info = {
        **info,
        "time_left": self._config.command_config.resample_time,
    }
    def new_command(info):
      current_goal_index = info["current_goal_index"] + 1
      # jax.debug.print("new command for step: {}, goal_index: {}", info["step"], current_goal_index)
      # jax.debug.print("goal achieved, new goal: {}", current_goal_index)
      desired_ee_positions_w = info["future_ee_positions_w"][current_goal_index]
      # jax.debug.print("resampled to new goal index: {}", current_goal_index)
      return {
            **info,
            "reach_goal_timer": 0,
            "current_goal_index": current_goal_index,
            "desired_ee_positions_w": desired_ee_positions_w,
            "resample_count": info["resample_count"] + 1,
        }    

    def old_command(info):
      # jax.debug.print("no new command for step: {}", info["step"])
      # jax.debug.print("still same goal")
      return info
      
    
    goal_reached = info["reach_goal_timer"] >= self.goal_reached_threshold

    # jax.debug.print("goal_reached: {}, index: {}", goal_reached, info["current_goal_index"])


    return jax.lax.cond(goal_reached, new_command, old_command, new_info)
  ...
  def _update_command(self, info: Dict) -> Dict:
    ...
    def increment_timer(info):
        # jax.debug.print("goal achieved for step: {}, incrementing timer: {}", info["step"], info["reach_goal_timer"] + 1)
        return {**info, "reach_goal_timer": info["reach_goal_timer"] + 1}

    def reset_timer(info):
        # jax.debug.print("goal not achieved for step: {}, reach goal timer was: {}", info["step"], info["reach_goal_timer"])
        return {**info, "reach_goal_timer": 0}
    ...
    # jax.debug.print("position command: {}, contact command: {}", info["position_command"], info["contact_command"])
    return info
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    info["time_left"] = 0.0
    info["previous_goal_index"] = jp.zeros(4)
    ...
    info["reach_goal_timer"] = 3
    info["desired_ee_positions_w"] = jp.zeros((2, 3))
    info["resample_count"] = 0
    info = self._compute_command(info)

    # data = data.replace(mocap_pos=data.mocap_pos.at[2].set(info["future_ee_positions_w"][0][0]))
    # data = data.replace(mocap_pos=data.mocap_pos.at[3].set(info["future_ee_positions_w"][0][1]))
    
    # data = data.replace(mocap_pos=data.mocap_pos.at[4].set(info["future_ee_positions_w"][1][0]))
    # data = data.replace(mocap_pos=data.mocap_pos.at[5].set(info["future_ee_positions_w"][1][1]))

    # data = data.replace(mocap_pos=data.mocap_pos.at[6].set(info["future_ee_positions_w"][2][0]))
    # data = data.replace(mocap_pos=data.mocap_pos.at[7].set(info["future_ee_positions_w"][2][1]))

    # data = data.replace(mocap_pos=data.mocap_pos.at[8].set(info["future_ee_positions_w"][3][0]))
    # data = data.replace(mocap_pos=data.mocap_pos.at[9].set(info["future_ee_positions_w"][3][1]))

    # data = data.replace(mocap_pos=data.mocap_pos.at[10].set(info["future_ee_positions_w"][4][0]))
    # data = data.replace(mocap_pos=data.mocap_pos.at[11].set(info["future_ee_positions_w"][4][1])) 
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    state.info["robot_base_pos_w"] = data.xpos[self._torso_body_id]
    state.info["robot_base_quat_w"] = data.xquat[self._torso_body_id]
    state.info["current_ee_positions_w"] = data.site_xpos[self._feet_site_id]
    ...
    data = data.replace(mocap_pos=data.mocap_pos.at[0].set(state.info["desired_ee_positions_w"][0]))
    data = data.replace(mocap_pos=data.mocap_pos.at[1].set(state.info["desired_ee_positions_w"][1]))


    # jax.debug.print("step: {},\ncurrent_goal_index: {},\ndesired_ee_positions: {},\nfuture_ee_positions[current_goal]: {},\ncurrent contact command: {}", 
    #                 state.info["step"], state.info["current_goal_index"], state.info["desired_ee_positions_w"], state.info["future_ee_positions_w"][state.info["current_goal_index"]], 
    #                 state.info["contact_command"])
  ...
  def _touchdown_reward(self, info: Dict) -> jax.Array:
    ...
    current_plan = info["contact_command"][0]  
    next_plan = info["contact_command"][1]
    time_left = info["time_left"]
    ...
    # jax.debug.print("touchdown_reward: {}", reward)
    # jax.debug.breakpoint()

    return sum(reward)
  
  def _touchdown_finegrained_reward(self, info: Dict, std: float = 0.2) -> jax.Array:
    contact_status = info["contact_status"]  
    desired_ee_pos = info["desired_ee_positions_w"]  
    current_ee_pos = info["current_ee_positions_w"]
    current_plan = info["contact_command"][0]  
    ...
    # jax.debug.print("touchdwon finegrained reward: {}", final_reward)
  ...
  def _swing_reward(self, info: Dict) -> jax.Array:
      contact = info["contact_status"]  # shape: [num_feet]
      planned_contact = info["contact_command"][0]  # shape: [num_feet]
      ...
      contact_reward = jp.where(contact == planned_contact, 2.0, -3.0)

      # Apply reward only during swing phase
      swing_reward = contact_reward * swing_phase
      final_reward = sum(swing_reward)

      # jax.debug.print("swing reward: {}", final_reward)

      # jax.debug.breakpoint()
      

      return final_reward  # sum across feet



  
  def _stance_reward(self, info: Dict, std: float=0.3) -> jax.Array:
    contact = info["contact_status"]
    stance_phase = info["contact_command"][0]
    is_correct_contact = jp.where(contact == stance_phase, True, False)

    feet_errors = jp.linalg.norm(info["desired_ee_positions_w"] - info["current_ee_positions_w"], axis=-1)
    final_reward = (
        (
            (is_correct_contact * jp.exp(-feet_errors / std**2))  
            + (~is_correct_contact * -0.1)
        )
        * stance_phase
    ).sum()

    # jax.debug.print("stance reward: {}", final_reward)
  ...
  def _new_goal_reward(self, info: Dict) -> jax.Array:
    ...
    goal_advanced = current_goal_index != previous_goal_index[4]
    reward = jp.where(goal_advanced, 1.0, 0.0)

    # jax.debug.print("step: {}, current goal index: {}, previous goal index[4]: {}, new goal reward: {}", info["step"], current_goal_index, previous_goal_index[4], reward)
    # jax.debug.breakpoint()
  ...
  def _cost_joint_deviation_hip(
    ...
    # )
    weight = jp.array([0.0, 1.0, 0.0, 1.0])
