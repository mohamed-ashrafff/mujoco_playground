...
class ContactEnv(g1_base.G1Env):
  ...
  def _generate_future_ee_positions(self, rng: jax.Array, data: mjx.Data, contact_plan: jax.Array) -> jax.Array:
    ...
    heading = jax.random.uniform(key, minval=-self.angle_range[0], maxval=self.angle_range[1])
    direction = jp.array([jp.cos(heading), jp.sin(heading), 0.0])

    rng, key = jax.random.split(rng)
    stride_length = jax.random.uniform(key, minval=self.stride_length_range[0], maxval=self.stride_length_range[1])
  ...
  def _convert_ee_to_base_frame(self, info):
        """
        ...
        Returns:
            jnp.ndarray: Future end-effector positions in robot base frame.
                Shape: (contact_horizon, 2, 3)
        """
        contact_horizon = self.contact_horizon
  ...
  def _compute_command(self, info: Dict) -> jax.Array:
    ...
    info = jax.lax.cond(time_left <= 0.0, lambda _: self._resample_command(info), lambda _: info, operand=None)
  ...
  def _resample_command(self, info: Dict) -> Dict:
    def new_command(info):
      ...
      current_goal_index = jax.lax.cond(current_goal_index >= self.contact_horizon, lambda _: 0, lambda _: current_goal_index, operand=None)
      info["current_goal_index"] = current_goal_index
      info["desired_ee_positions_w"] = info["future_ee_positions_w"][current_goal_index]
      return info    
  ...
  def _update_command(self, info: Dict) -> Dict:
    info = self._convert_ee_to_base_frame(info)
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    data = data.replace(site_xpos=data.site_xpos.at[self.left_foot_target_site].set([5.0, 0.0, 0.0]))
    data = data.replace(site_xpos=data.site_xpos.at[self.right_foot_target_site].set([3.0, 0.0, 0.0]))
  ...
  def _get_obs(
    ...
    state = jp.hstack([
        ...
        # jp.ravel(phase),                  # Uncomment and flatten if needed
    ])
...
#   def _stance_reward(self, info: Dict) -> jax.Array:
#     contact = info["contact"]
#     is_correct_contact = jax.lax.cond(contact[0] and info["contact_command"][0, 0] and  , lambda _: True, lambda _: False)
