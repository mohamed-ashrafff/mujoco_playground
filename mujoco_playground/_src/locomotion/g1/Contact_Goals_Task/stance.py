...
class Stance(g1_base.G1Env):
  ...
  def _post_init(self) -> None:
    ...
    self.left_foot_target_id = self._mj_model.geom("left_foot_target").id
    self.right_foot_target_id = self._mj_model.geom("right_foot_target").id
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    cmd = self.sample_command(cmd_rng, data)
    

    data = data.replace(geom_xpos=data.geom_xpos.at[self.left_foot_target_id].set(cmd[0]))
    data = data.replace(geom_xpos=data.geom_xpos.at[self.right_foot_target_id].set(cmd[1]))
    ...
    # Sample push interval.feet_offset
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    data = state.data.replace(geom_xpos=data.geom_xpos.at[self.left_foot_target_id].set(state.info["command"][0]))
    data = state.data.replace(geom_xpos=data.geom_xpos.at[self.right_foot_target_id].set(state.info["command"][1]))
    ...
    state.info["command"] = state.info["command"]
  ...
  def sample_command(self, rng: jax.Array, data: mjx.Data) -> jax.Array:
    initial_foot_positions = data.site_xpos[self._feet_geom_id]
    return initial_foot_positions
  
#   def update_goal_sites(self, cmd, data):
#     """
...
#     # updated_site_xpos = data.site_xpos.at[left_site_id].set(cmd[0])
#     # updated_site_xpos = data.site_xpos.at[right_site_id].set(cmd[1])

#     self._mj_model.site_pos[left_site_id] = cmd[0]
#     self._mj_model.site_pos[right_site_id] = cmd[1]

#     # Recompute derived quantities if needed
#     mujoco.mj_forward(self._mj_model, data)

#     # data = dataclasses.replace(data, site_xpos=updated_site_xpos)
#     # return data
  def update_goal_sites_in_model(
    self, model: mjx.Model, foot_positions: jax.Array
  ) -> mjx.Model:
    site_pos = model.site_pos
    site_pos = site_pos.at[self._left_goal_site_id].set(foot_positions[0])
    site_pos = site_pos.at[self._right_goal_site_id].set(foot_positions[1])
    return model.replace(site_pos=site_pos)